...
float* m_div(struct matrix,struct matrix);      // completed
float* m_adj(struct matrix);                    // feature unavaible
float* m_rank(struct matrix);                   // SCRAP Fn
...
float* m_transpose(struct matrix M1)
{
  ...
  ctr=0;
  //dequeue into new shape
  //printf("Emptying queue\n");
  for(int i=0;i<row_size;i++)
  {
    for(int j=0;j<colm_size;j+=2)
    {
      A[i][j]=queue[ctr];
      A[i][j+1]=queue[ctr+M1.colm_size];
    //  printf("%f\t",&queue[ctr]);
      ctr++;
    }
    //printf("\n");
  }
  //printf("FLAG1:EXITING TRANS Fn");
  ans_ptr=A;
...
float m_det(float M1[50][50],int n)
{
  ...
  for(int i=0;i<n;i++)
  {
    ...
    /*TO PRINT CONTENT OF M1 WHICH IS THE COFAC Matrix
    printf("CONTENTS OF M1\n");
